package com.jizhangyl.application.utils;//package trackingMore;
...
public class TrackerUtil {
     ...
     * Apikey
     */
    private String Apikey = "9500d802-51cd-4979-8101-dd1f531779e8";
    ...
    public static JSONObject getTrace(String expNum, String expAbbr) {
        try {
            String urlStr = null;
            String requestData = "{\"tracking_number\": " + "\"" + expNum + "\"" + ",\"carrier_code\":" + "\"" + expAbbr + "\"" + ",\"destination_code\":\"US\",\"tracking_ship_date\": \"20180226\",\"tracking_postal_code\":\"13ES20\",\"specialNumberDestination\":\"US\",\"order\":\"#123123\",\"order_create_time\":\"2018/3/27 16:51\",\"lang\":\"en\"}";
            String result = new TrackerUtil().orderOnlineByJson(requestData, urlStr, "realtime");
            JSONObject jsonResult = JSONObject.parseObject(result);
            JSONObject data = (JSONObject) jsonResult.get("data");
            if (data == null || data.size() == 0) {
                throw new GlobalException(ResultEnum.EXPRESS_QUERY_FAIL);
            }
            JSONArray items = (JSONArray) data.get("items");
            JSONObject firstItem = (JSONObject) items.get(0);
            JSONObject destinationInfo = (JSONObject) firstItem.get("destination_info");
            return destinationInfo;
        } catch (Exception e) {
            log.error("【物流查询】{}", e.getMessage());
            throw new GlobalException(ResultEnum.EXPRESS_QUERY_FAIL.getCode(), e.getMessage());
        }
    }

    public static void main(String[] args) {
        try {
            String expNum = "EJ671019253JP";
            String expAbbr = "japan-post";
            String urlStr = null;
            String requestData = "{\"tracking_number\": " + "\"" + expNum + "\"" + ",\"carrier_code\":" + "\"" + expAbbr + "\"" + ",\"destination_code\":\"US\",\"tracking_ship_date\": \"20180226\",\"tracking_postal_code\":\"13ES20\",\"specialNumberDestination\":\"US\",\"order\":\"#123123\",\"order_create_time\":\"2018/3/27 16:51\",\"lang\":\"en\"}";
            String result = new TrackerUtil().orderOnlineByJson(requestData, urlStr, "realtime");
            JSONObject jsonResult = JSONObject.parseObject(result);
            ...
            JSONObject firstItem = (JSONObject) items.get(0);
            JSONObject destinationInfo = (JSONObject) firstItem.get("destination_info");
            JSONArray destinationTraceInfo = (JSONArray) destinationInfo.get("trackinfo");
            log.info("destinationTraceInfo = {}", destinationTraceInfo);
        } catch (Exception e) {
            log.error("【物流查询】{}", e.getMessage());
            throw new GlobalException(ResultEnum.EXPRESS_QUERY_FAIL.getCode(), e.getMessage());
        }
